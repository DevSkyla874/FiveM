...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        if (args.give) {
            ...
            // Try QBCore.Shared.Vehicles first, fallback to using model name directly
            const vehicles = client.QBCore.Shared.Vehicles;
            const vehicle = vehicles ? vehicles[Object.keys(vehicles).find(key => key.toLowerCase() === model) || ""] : null;
            const vehicleName = vehicle ? vehicle.name : model;
            const vehicleHash = vehicle ? vehicle.hash : GetHashKey(model);
            ...
            const save = await global.exports.oxmysql.insert_async("INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, state, garage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [
                player.PlayerData.license, player.PlayerData.citizenid, model, vehicleHash, "{}", plate, 1, "pillboxgarage",
            ]);
            if (!save) return interaction.reply({ content: "Something went wrong saving the vehicle", ephemeral: true });

            client.utils.log.info(`[${interaction.member.displayName}] Gave ${GetPlayerName(args.id)} (${args.id}) a ${model} with the plate ${plate}`);
            const playerMessage = `${vehicleName} has been added to your garage (Plate: ${plate})`;

            // Use ox_lib notify instead of deprecated QBCore:Notify
            ...
            return interaction.reply({ content: `${GetPlayerName(args.id)} (${args.id}) was given a ${vehicleName}. Plate: ${plate}`, ephemeral: false });
        }
        else if (args.lookup) {
